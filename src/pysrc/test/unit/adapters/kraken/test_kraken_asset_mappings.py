...
def test_asset_conversion() -> None:
    for asset in Asset:
        ...
        assert (
            kraken_to_asset(asset_to_kraken(asset, Market.KRAKEN_USD_FUTURE)) == asset
        )

        assert (
            kraken_to_market(asset_to_kraken(asset, Market.KRAKEN_SPOT))
            == Market.KRAKEN_SPOT
        )
        assert (
            kraken_to_market(asset_to_kraken(asset, Market.KRAKEN_USD_FUTURE))
